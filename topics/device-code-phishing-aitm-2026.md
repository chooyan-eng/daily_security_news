...
OAuth 2.0 デバイス認証フロー（Device Authorization Grant）を悪用してセッショントークンを直接窃取し、パスワードや MFA を回避する AiTM（Adversary-in-the-Middle）攻撃手法。2026年6月に37倍の検出スパイクを記録し、18種類のフィッシングキットが登場、すべての主要 AiTM プラットフォームが採用したことで犯罪エコシステムへのコモディティ化が確認された。Microsoft 365・Azure AD・Google Workspace 等のクラウドプラットフォームが主な標的。2026年7月には、フィッシングページをAES-GCMで暗号化しブラウザ内復号まで正体を隠す「ゴーストフィッシング」を用いる新キャンペーン「EvilTokens」が確認され、静的URL解析による検知回避という新たな技術トレンドが加わった。
